...
public class ConfigParser {
    ...
    public VpnProfile convertProfile() throws ConfigParseError, IOException {
        ...
        if (secret != null) {
            np.mAuthenticationType = VpnProfile.TYPE_STATIC_KEYS;
            noauthtypeset = false;
            np.mUseTLSAuth = true;
        ...
        if (tlsremote != null) {
            ...
            np.mX509AuthType = VpnProfile.X509_VERIFY_TLS_REMOTE;

            if ((compatnames != null && compatnames.size() > 2) ||
                    (nonameremapping != null))
                np.mX509AuthType = VpnProfile.X509_VERIFY_TLS_REMOTE_COMPAT_NO_REMAPPING;
        }
        ...
        if (verifyx509name != null) {
            ...
            if (verifyx509name.size() > 2) {
                if (verifyx509name.get(2).equals("name"))
                    np.mX509AuthType = VpnProfile.X509_VERIFY_TLS_REMOTE_RDN;
                else if (verifyx509name.get(2).equals("subject"))
                    np.mX509AuthType = VpnProfile.X509_VERIFY_TLS_REMOTE_DN;
                else if (verifyx509name.get(2).equals("name-prefix"))
                    np.mX509AuthType = VpnProfile.X509_VERIFY_TLS_REMOTE_RDN_PREFIX;
                else
        ...
        if (authuser != null) {
            if (noauthtypeset) {
                np.mAuthenticationType = VpnProfile.TYPE_USER_PASS;
            } else if (np.mAuthenticationType == VpnProfile.TYPE_CERTIFICATES) {
                np.mAuthenticationType = VpnProfile.TYPE_USER_PASS_CERTIFICATES;
            } else if (np.mAuthenticationType == VpnProfile.TYPE_KEYSTORE) {
                np.mAuthenticationType = VpnProfile.TYPE_USER_PASS_KEYSTORE;
            }
            if (authuser.size() > 1) {
