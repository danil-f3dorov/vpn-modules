...
public class VpnProfile implements Serializable, Cloneable {
    ...
    public static final int TYPE_PKC_S12 = 1;
    public static final int TYPE_KEYSTORE = 2;
    public static final int TYPE_USER_PASS = 3;
    public static final int TYPE_STATIC_KEYS = 4;
    public static final int TYPE_USER_PASS_CERTIFICATES = 5;
    public static final int TYPE_USER_PASS_PKC_S12 = 6;
    public static final int TYPE_USER_PASS_KEYSTORE = 7;
    public static final int X509_VERIFY_TLS_REMOTE = 0;
    public static final int X509_VERIFY_TLS_REMOTE_COMPAT_NO_REMAPPING = 1;
    public static final int X509_VERIFY_TLS_REMOTE_DN = 2;
    public static final int X509_VERIFY_TLS_REMOTE_RDN = 3;
    public static final int X509_VERIFY_TLS_REMOTE_RDN_PREFIX = 4;
    ...
    public String mAuth = "";
    public int mX509AuthType = X509_VERIFY_TLS_REMOTE_RDN;
    ...
    public String getConfigFile(Context context, boolean configForOvpn3) {
        ...
        boolean useTLSClient = (mAuthenticationType != TYPE_STATIC_KEYS);
        ...
        switch (mAuthenticationType) {
            case VpnProfile.TYPE_USER_PASS_CERTIFICATES:
                cfg += "auth-user-pass\n";
            case VpnProfile.TYPE_CERTIFICATES:
                ...
                break;
            case VpnProfile.TYPE_USER_PASS_PKC_S12:
                cfg += "auth-user-pass\n";
            case VpnProfile.TYPE_PKC_S12:
                cfg += insertFileData("pkcs12", mPKCS12Filename);
                break;

            case VpnProfile.TYPE_USER_PASS_KEYSTORE:
                cfg += "auth-user-pass\n";
            case VpnProfile.TYPE_KEYSTORE:
                ...
                break;
            case VpnProfile.TYPE_USER_PASS:
                cfg += "auth-user-pass\n";
                cfg += insertFileData("ca", mCaFilename);
        }
        ...
        if (mUseTLSAuth) {
            if (mAuthenticationType == TYPE_STATIC_KEYS)
            ...
            if (mCheckRemoteCN) {
                if (mRemoteCN == null || mRemoteCN.equals(""))
                    ...
                    switch (mX509AuthType) {
                        ...
                        case X509_VERIFY_TLS_REMOTE_COMPAT_NO_REMAPPING:
                            cfg += "compat-names no-remapping\n";
                        case X509_VERIFY_TLS_REMOTE:
                            cfg += "tls-remote " + openVpnEscape(mRemoteCN) + "\n";
                            break;

                        case X509_VERIFY_TLS_REMOTE_RDN:
                            cfg += "verify-x509-name " + openVpnEscape(mRemoteCN) + " name\n";
                            break;

                        case X509_VERIFY_TLS_REMOTE_RDN_PREFIX:
                            cfg += "verify-x509-name " + openVpnEscape(mRemoteCN) + " name-prefix\n";
                            break;

                        case X509_VERIFY_TLS_REMOTE_DN:
...
//        }
    ...
    public static String getDisplayName(String embeddedFile) {
        int start = DISPLAY_NAME_TAG.length();
    ...
    public static boolean isEmbedded(String data) {
        if (data == null)
            return false;
        return data.startsWith(INLINE_TAG) || data.startsWith(DISPLAY_NAME_TAG);
    }

    public void checkForRestart(final Context context) {
        /* This method is called when OpenVPNService is restarted */

        if ((mAuthenticationType == VpnProfile.TYPE_KEYSTORE || mAuthenticationType == VpnProfile.TYPE_USER_PASS_KEYSTORE)
                && mPrivateKey == null) {
...
//            /data/data/com.vasilkoff.easyvpnfree/lib
//            /data/data/com.vasilkoff.easyvpnfree/files
    ...
    public int checkProfile(Context context) {
        if (mAuthenticationType == TYPE_KEYSTORE || mAuthenticationType == TYPE_USER_PASS_KEYSTORE) {
            if (mAlias == null)
                return R.string.no_keystore_cert_selected;
        }

        if (!mUsePull || mAuthenticationType == TYPE_STATIC_KEYS) {
        ...
        if ((mAuthenticationType == TYPE_USER_PASS_CERTIFICATES || mAuthenticationType == TYPE_CERTIFICATES)
                && (TextUtils.isEmpty(mClientCertFilename) || TextUtils.isEmpty(mClientKeyFilename)))
            return R.string.missing_certificates;

        if ((mAuthenticationType == TYPE_CERTIFICATES || mAuthenticationType == TYPE_USER_PASS_CERTIFICATES)
    ...
    public String getPasswordPrivateKey() {
        ...
        switch (mAuthenticationType) {
            case TYPE_PKC_S12:
            case TYPE_USER_PASS_PKC_S12:
                return mPKCS12Password;

            case TYPE_CERTIFICATES:
            case TYPE_USER_PASS_CERTIFICATES:
                return mKeyPassword;

            case TYPE_USER_PASS:
            case TYPE_STATIC_KEYS:
            default:
                return null;
        }
    }
    ...
    public int needUserPWInput(boolean ignoreTransient) {
        if ((mAuthenticationType == TYPE_PKC_S12 || mAuthenticationType == TYPE_USER_PASS_PKC_S12) &&
